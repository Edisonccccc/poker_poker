...
import { AfterGamePanel } from "@/features/afterGame/AfterGamePanel";
...
export function GameDetailPage() {
  ...
  if (isError || !game)
    return <p className="text-sm text-amber-400">Couldn't load game.</p>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex rounded-xl bg-white/5 p-1">
        {(
          ...
          <button
            key={key}
            onClick={() => setTab(key)}
            className={`min-h-tap flex-1 rounded-lg py-2 text-sm font-semibold ${
      ...
      {tab === "after" && <AfterGamePanel gameId={gameId} />}
      {tab === "stats" && (
...
function TableRow({ table }: { table: TableSummary }) {
  return (
    <li>
      <Link to={`/tables/${table.id}`} className="block rounded-2xl bg-white/5 p-4">
        ...
        <div className="mt-1 text-sm text-white/55">
          ...
          {table._count.denominations} chip color
          {table._count.denominations === 1 ? "" : "s"} ·{" "}
